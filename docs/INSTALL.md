...
`@local/systems-slides-template:0.5.0`，底层演示框架始终来自官方
...
```text
<package-root>/local/systems-slides-template/0.5.0/
```
...
初始化出的 `globals.typ` 继续导入同一个 `@local/systems-slides-template:0.5.0`，因此项目
...
```sh
make package          # build/dist/packages/local/systems-slides-template/0.5.0/
make package-check    # 生成并校验发行快照
make package-stage    # build/packages/local/systems-slides-template/0.5.0/
...
### 找不到 `@local/systems-slides-template:0.5.0`
